...
| D4 | Focus 扁平无层级 | P1 | 粒度两难，缺 focus↔focus 关系（⏸ 2026-07-09 需求验证：暂不建，见 §5c） |
| D5 | 隐私全局单档 | P1 | 缺 per-source 粒度（open question #5，实为刚需）（✅ 已于 2026-07-08 落地） |
| D6 | 缺时间序模型 | P1 | 按摄取序而非 occurredAt，趋势/活跃度算错（✅ 已于 2026-07-08 落地：活跃度用 occurredAt、runs 按 occurred_at 排序、`fie trend`/`GET /v1/trend` 按 occurredAt 日聚合） |
...
## 5c. D4 需求验证：Focus 层级（结论：暂不建）

> 验证时间：2026-07-09。结论：**D4 需求当前不成立，维持"规划中"，不进入实现。** 以下为验证依据与再评估触发条件。

### 目标
判断"Focus 扁平无层级"是否是需要现在解决的真实问题，避免在未验证需求前引入 Focus↔Focus 关系这类高表面积改动（违背"收敛优先"与不做投机性设计）。

### 验证依据

1. **"粒度两难"拆成两个方向，均已被现有能力覆盖或部分覆盖：**
   - *太细（碎片化）*：已闭合。收敛优先 + 双阈值 + `low_confidence` 复核（D1）抑制新建；事后可 `merge`；`paths_json` 跨工具收敛（D2）在文件维度归并。
   - *项目级 roll-up 视图*：已部分可行。`focuses.project` 是事实上的分组键，多个 Focus 共享同一 `project` 即可按项目聚合，**无需层级表**；`fie trend`/`stats`（D6/D3）提供可观测性。
2. **真正只有层级才能解决的场景仅两个，且都非核心价值：**
   - 匹配传播（子命中回溯父上下文）——§11 自述"不改变归因主链路"，本身不影响核心价值。
   - 跨 project / 任意深度主题分组——`project` 单字符串表达不了，但属边缘诉求。
3. **需求证据：无。** 系统尚未真实使用（连阈值校准都缺真实语料）。目标用户核心诉求（§3/§4：近期关注了什么、主题漂移、反复阻塞）无一需要父子树；"项目级视图"用 `project` 分组 + trend 已服务。
4. **现在建的代价：** 需动迁移（`parent_focus_id`）、CLI、匹配传播决策，且必须定义父被 merge/archive/dormant 时子的处置语义——一大片表面积服务未验证需求。

### 再评估触发条件（真实使用后采集，多数已可观测）
- `focus_events` 出现"为归组而 merge"的模式（想保留子身份却只能粗暴合并）。
- 单一 `project` 下 Focus 数持续膨胀且项目视图噪音大（`fie trend`/`stats` 可观测）。
- 出现 `project` 字段表达不了的跨 project 主题诉求。

### 若验证成立时的最小楔子（非现在做）
先做零 schema 的"按 project 分组"只读视图（如 `focus list --by-project`）检验"project 分组是否已够用"；仍不够才引入单个可空 `parent_focus_id` 自引用 + 视图 roll-up，**不做匹配传播、不做自动层级推断**，远小于 §11 完整愿景。
